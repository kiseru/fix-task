...
public class CountServletTest {
    ...
    public void testGetMoves() {
        var uri = String.format(URI_TEMPLATE, CountServlet.URI, 14, 10, "A3", "B1");
    ...
    public void testGetMovesWithoutOneParameter() {
        var uriWithoutWidthParameter = String.format("%s?&height=%d&start=%s&end=%s", CountServlet.URI, 10, "A3", "B1");
        assertEquals(HttpStatus.BAD_REQUEST,
                restTemplate.getForEntity(URI.create(uriWithoutWidthParameter), String.class).getStatusCode());

        var uriWithoutHeightParameter = String.format("%s?width=%d&start=%s&end=%s", CountServlet.URI, 14, "A3", "B1");
        assertEquals(HttpStatus.BAD_REQUEST,
                restTemplate.getForEntity(URI.create(uriWithoutHeightParameter), String.class).getStatusCode());

        var uriWithoutStartParameter = String.format("%s?width=%d&height=%d&end=%s", CountServlet.URI, 14, 10, "B1");
        assertEquals(HttpStatus.BAD_REQUEST,
                restTemplate.getForEntity(URI.create(uriWithoutStartParameter), String.class).getStatusCode());

        var uriWithoutEndParameter = String.format("%s?width=%d&height=%d&start=%s", CountServlet.URI, 14, 10, "A3");
        assertEquals(HttpStatus.BAD_REQUEST,
    ...
    public void testGetMovesWithInvalidParameter() {
        var uriWithWrongParameter = String.format("%s?width=%s&height=%d&start=%s&end=%s", CountServlet.URI, "A", 10,
                "A3", "B1");
    ...
    public void testGetMovesWithInvalidStartPosition() {
        var uriWithInvalidStartPosition = String.format(URI_TEMPLATE, CountServlet.URI, 3, 3, "A4", "B1");
    ...
    public void testGetMovesWithInvalidEndPosition() {
        var uriWithInvalidEndPosition = String.format(URI_TEMPLATE, CountServlet.URI, 3, 3, "A1", "B4");
